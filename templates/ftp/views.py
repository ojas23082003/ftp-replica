		...
		try:
			...
			for s in status:
				...
				if statusId + s[1] not in profile.viewedNotifications.split(','):
					...
					profile.viewedNotifications = profile.viewedNotifications + statusId + s[1] + ','
					profile.save()
...
def gallery_hultprize2021(request):
	return render(request, 'home/gallery/hultprize2021.html')
...
def index(request):
	...
	if not notices:
		success = False
	return render(request, 'ftp/home/index_old.html', {'success': success, 'pendingNotifications': pendingNotifications, 'notices': notices})
...
def register(request):
	...
	if request.method == 'POST':
		...
		try:
			query = CustomUser.objects.get(username=request.POST.get("username"))
...
def view_projects_new(request):
	...
	if not request.user.is_authenticated:
		...
		print(success)
		return render(request, 'ftp/ProjectsPage/projects_new.html',
					  {"success": success, 'allprojects': rem_project, 'prof': profile, 'bookmarks': bookmarked,'pastproject':pastproject, 'proj_domains': proj_domains, 'date':current_time, 'selected':profile.selected, "pendingNotifications": pendingNotifications})
...
def view_results(request):
	...
	if not request.user.is_authenticated:
		...
		return render(request, 'ftp/results/results_new.html', {'prof': profile,'result':res, "pendingNotifications": pendingNotifications})
...
def already_applied(request):
	...
	if not request.user.is_authenticated:
		...
		for app in applied_projects_app:
			success = False
			status.append([app.project, app.status])
			applied_projects.append([app.project, app.sop, app.ncv, app.project_domain1, app.project_domain2, app.project_domain3, app.Loi,app.status])
...
def applied_students(request, project_id):
	...
	all_applied = Profile.objects.filter(application__project=proj, application__display_prof=True)
	success = False
	...
	if not request.user.is_authenticated:
		...
		try:
			...
			if project.display:
				...
				Application.objects.create(profile=profile, project=project, project_domain1=project_domain1, project_domain2=project_domain2, project_domain3=project_domain3, sop=sop, ncv=ncv, Loi=Loi, noc=noc)
				messages.success(request, 'Applied successfully!')
				return redirect('ftp:projects')
			else:
				# logout(request)
...
def apply_scholarship(request):
	if request.method == 'POST':
		...
		scholarship_id = request.POST.get('col1')
...
def edit_profile(request):
	...
	if request.method == 'POST':
		...
		return render(request, 'ftp/profile/edit_profile.html', {'profile': profile, "pendingNotifications": pendingNotifications})
...
def view_favorite(request):
	...
	if not request.user.is_authenticated:
		...
		if profile is None:
			...
			current_time = datetime.datetime.now().date()
			return render(request, 'ftp/ProjectsPage/bookmarks_new.html',
...
def view_profile(request):
	...
	if not request.user.is_authenticated:
		...
		return render(request, 'ftp/profile/view_profile_new.html', {"prof": profile, "pendingNotifications": pendingNotifications})
...
def dashboard_mail(request):
		...
		if request.method == 'POST':
			...
			try:
				email2 = EmailMultiAlternatives(
					subject, message, settings.EMAIL_HOST_USER, to=[email,alt_email]
				)
			except:
				email2 = EmailMultiAlternatives(
					subject, message, settings.EMAIL_HOST_USER, to=[email,alt_email]
			...
			if typeofmail == "2":
				email2.attach_file(os.path.join(settings.BASE_DIR, 'media/Undertaking.pdf'))
			try:
			...
			if error_message != "No Errors":
				messages.success(request, 'All email were not sent successfully. Check logs.')
			else:
	...
	if request.GET.get('search'):
		...
		projects = Project.objects.filter(Q(project_name__icontains=search) | Q(professor_name__icontains=search) | Q(university__icontains=search))
	profile = get_object_or_404(Profile, username=request.user)
	display=[]
	display_past=[]
	...
	return render(request, 'ftp/admin/projects.html', {'displaylist': display, 'displaylist_past': display_past, 'allprofessors': professors, 'prof': profile, 'projects_declared': projects_declared})
...
def admin_projects(request):
	...
	if user.is_irc:
		...
		if request.GET.get('pname'):
			...
			# logfile.close()
			if error_message != "No Errors":
				messages.success(request, 'All emails were not sent successfully. Check logs.')
			else:
		...
		for result in results:
			projects_declared.append(result.project)
		print(projects_declared)
		return render(request, 'ftp/admin/projects.html', {'displaylist': display, 'displaylist_past': display_past, 'allprofessors': professors, 'prof': profile, 'projects_declared': projects_declared ,'projtag':project_tag})
	else:
		raise Http404('Page does not Exist')
...
def admin_profiles(request):
	...
	if user.is_irc:
		...
		profile = get_object_or_404(Profile, username=request.user)
		# print(profiles)
		profiles=[i for i in profiles] # Converting query set object into list
...
def export_profile(request):
	if request.user.is_irc:
		...
		writer.writerow(['username', 'fullname', 'department', 'contact','year', 'email','alt_email','passport','cgpa','rollno', 'alt_email','selected'])
		profile = Profile.objects.all().values_list('username', 'fullname', 'department', 'contact','year','alt_email','passport','cgpa', 'rollno', 'alt_email','selected')
...
def export_mail_ids(request):
	if request.user.is_irc:
		...
		profile = Profile.objects.all().values_list('fullname', 'alt_email', 'username_id')
		for each_profile in profile:
...
def export_professor(request):
	if request.user.is_irc:
		...
		profile = Professor.objects.all().values_list('name', 'university_name', 'project_name', 'skype')
		for each_profile in profile:
			writer.writerow(each_profile)
		return response
	else:
...
def export_saip(request):
	if request.user.is_irc:
		...
		profile = SAIP.objects.all().values_list('fullname', 'rollno', 'email', 'query', 'mode_of_saip')
		for each_profile in profile:
			writer.writerow(each_profile)
		return response
	else:
...
def export_hult(request):
	if request.user.is_irc:
		...
		writer = csv.writer(response)
		writer.writerow(['team_name', 'email_leader', 'member_2', 'email_mem2','member_3','email_mem3','member_4','email_mem4','queries'])
		hult = Hult.objects.all().values_list('team_name', 'email_leader', 'member_2', 'email_mem2','member_3','email_mem3','member_4','email_mem4','queries')
...
def export_dyuti(request):
	if request.user.is_irc:
		...
		writer = csv.writer(response)
		writer.writerow(['first_name', 'last_name', 'phone', 'email','gender','country','passport_no','visa_no','visa_cate','visa_validity','uni_india','state','city','department','year_of_study','reg_no','is_scholarship'])
		dyuti = Dyuti.objects.all().values_list('first_name', 'last_name', 'phone', 'email','gender','country','passport_no','visa_no','visa_cate','visa_validity','uni_india','state','city','department','year_of_study','reg_no','is_scholarship')
...
def export_applied_list(request,pk):
	if request.user.is_irc:
		...
		writer = csv.writer(response)
		writer.writerow(['Username', 'Fullname', 'Roll No', 'Contact No', 'CGPA', 'Department', 'CV', 'TRANSCRIPT', 'SOP', 'Letter of Importance'])

		for each in query_set:
			if each.ncv:
				writer.writerow([each.profile.username, each.profile.fullname, each.profile.rollno, each.profile.contact, each.profile.cgpa, each.profile.department, 'https://ircell.iitkgp.ac.in' + each.ncv.url, 'https://ircell.iitkgp.ac.in' + each.profile.transcript.url, each.sop, each.Loi])
			else:
				writer.writerow([each.profile.username, each.profile.fullname, each.profile.rollno, each.profile.contact, each.profile.cgpa, each.profile.department, 'https://ircell.iitkgp.ac.in' + each.profile.cv.url,'https://ircell.iitkgp.ac.in' + each.profile.transcript.url, each.sop, each.Loi])
		return response
	else:
		raise Http404('Page does not Exist')
...
def export_gkf(request):
	if request.user.is_irc:
		...
		writer = csv.writer(response)
		writer.writerow(['Full Name', 'Roll No', 'Email', 'Contact', 'Student', 'Year of Study', 'Department', 'CGPA', 'Transcript', 'CV', 'SOP', 'Type', 'Offer Letter', 'NOC', 'From', 'To', 'Host University', 'Previously Funded', 'Funding Proof'])

		for each in query_set:
			if each.funding_proof:
				writer.writerow([each.name, each.roll, each.email, each.contact, each.student, each.yearOfStudy, each.department, each.cgpa,  'https://ircell.iitkgp.ac.in' + each.transcript.url, 'https://ircell.iitkgp.ac.in' + each.cv.url, each.sop, each.type,  'https://ircell.iitkgp.ac.in' + each.offer_letter.url, 'https://ircell.iitkgp.ac.in' + each.noc.url, 'D: ' + each.durationOfVisit_from, 'D: ' + each.durationOfVisit_to, each.hostUniversity, each.previously_funded, 'https://ircell.iitkgp.ac.in' + each.funding_proof.url])
			else:
				writer.writerow([each.name, each.roll, each.email, each.contact, each.student, each.yearOfStudy, each.department, each.cgpa,  'https://ircell.iitkgp.ac.in' + each.transcript.url, 'https://ircell.iitkgp.ac.in' + each.cv.url, each.sop, each.type,  'https://ircell.iitkgp.ac.in' + each.offer_letter.url, 'https://ircell.iitkgp.ac.in' + each.noc.url, 'D: ' + each.durationOfVisit_from, 'D: ' + each.durationOfVisit_to, each.hostUniversity, each.previously_funded, "Not Funded!"])
		return response
	else:
		raise Http404('Page does not Exist')
...
def export_applied_anu_list(request,project_name):
	if request.user.is_irc:
		query_set = Anu.objects.filter(Q(pref1=project_name) | Q(pref2=project_name) | Q(pref3=project_name))

		response = HttpResponse(content_type='text/csv')
		response['Content-Disposition'] = 'attachment; filename=" ' + project_name + ' List.csv"'
		writer = csv.writer(response)
		...
		for each in query_set:
			writer.writerow([each.fullname, each.rollno, each.email, each.year, each.contact, each.sop, each.former_anu,'https://ircell.iitkgp.ac.in' + each.lor1.url, 'https://ircell.iitkgp.ac.in' + each.lor2.url, 'https://ircell.iitkgp.ac.in' + each.undertaking.url, 'https://ircell.iitkgp.ac.in' + each.cv.url, 'https://ircell.iitkgp.ac.in' + each.transcript.url, each.pref1, each.pref2, each.pref3])
		return response
	else:
		raise Http404('Page does not Exist')
...
def project_create(request):
	...
	if user.is_irc:
		...
		try:
			prereq = PreRequisite.objects.get(detail=request.POST.get('prereq_detail'))
		except PreRequisite.DoesNotExist:
			prereq = PreRequisite.objects.create(detail=request.POST.get('prereq_detail'))
		...
		if form.is_valid():
			...
			form.save()
			print("success")
			proj = Project.objects.get(project_name=request.POST.get('project_name'))
			...
			for p in request.POST.getlist('form.tags'):
				Project.objects.get(project_name=request.POST.get('project_name')).tags.add(str(p))
			...
			if request.POST.get('update')=='on':
				...
				for each_user in active_users:
					...
					try:
						alt_email = Profile.objects.filter(username=each_user).alt_email
						email = EmailMessage(
							subject, message, settings.EMAIL_HOST_USER, to=[each_user.email, alt_email]
						)
					except:
						email = EmailMessage(
...
def hult(request):
	if(request.method == "POST"):
		...
		if form.is_valid():
			form.save()
			messages.success(request, 'Applied successfully!')
			# return HttpResponse('<h1>Badiya</h1>')
			return redirect('hult')
		else:
			print(form.errors)
			return render(request, 'ftp/hult.html', {'form':form,'errors':form.errors})
	else:
		form = HultForm()
		return render(request, 'ftp/hult.html',{'form':form})	
	return render(request, 'ftp/hult.html')
...
def admin_professors(request):
	...
	if user.is_irc:
		...
		if request.GET.get('search'):
			search = request.GET.get('search')
			allprofs = Professor.objects.filter(Q(project_name__icontains=search) | Q(name__icontains=search) | Q(university_name__icontains=search))
...
class ProjectUpdate(UpdateView):
	...
	fields = ('professor_name','university','project_name','project_detail','project_time','display','special','deadline', 'stipend','tags')
	success_url = reverse_lazy('ftp:admin_projects')
...
class ProfessorUpdate(UpdateView):
	...
	fields = ('name', 'university_name', 'project_name', 'project_details', 'prerequisites', 'no_of_students', 'duration','stipend', 'skype', 'imported')
...
def search_profile(request):
	if request.method == 'GET':
		search = request.GET.get('search')
		status = Profile.objects.filter(Q(fullname__icontains=search) | Q(rollno__icontains=search) | Q(department__icontains=search)) # filter returns a list so you might consider skip except part
		print(len(status))
		if not status:
			print("Empty")
...
class ProfileUpdate(UpdateView):
	...
	fields = ['fullname', 'department', 'contact', 'year', 'alt_email', 'passport', 'transcript', 'cgpa', 'cv', 'photo', 'review', 'rollno']
...
def import_project(request):
	...
	if len(projects) == 0:
		try:
			...
			prereq = PreRequisite.objects.create(detail=professor.prerequisites)
		project = Project.objects.create(professor_name=professor.name, university=professor.university_name,project_name=professor.project_name, project_detail=professor.project_details,prerequisite=prereq, stipend=professor.stipend,project_time=professor.duration)    
		data = {'fb': True}
...
def feedback_form(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'FeedBack submitted successfully successfully!')
...
def showstatus(request,project_id):
	...
	if user.is_irc:
		...
		if request.method=="POST":
			...
			msg=Profile.objects.get(id=name).rollno+" Updation Successfull to " + ustatus.upper()
...
def project_csv(request):
	...
	if user.is_irc:
		...
		writer.writerow(['professor_name', 'university', 'project_name', 'project_detail', 'project_mode', 'project_time', 'deadline', 'prerequisite', 'stipend', 'tags'])
		...
		for p in project:
			...
			writer.writerow([p.professor_name, p.university, p.project_name, p.project_detail, p.project_mode, p.project_time, p.deadline, p.prerequisite, p.stipend, lis])
...
def professor_csv(request):
	...
	if user.is_irc:
		...
		writer.writerow(['professor_name', 'university_name', 'project_name', 'no_of_students', 'prerequisites', 'duration', 'stipend', 'skype', 'created_at'])
		
		
		for p in professor:
			writer.writerow([p.name, p.university_name, p.project_name, p.no_of_students, p.prerequisites, p.duration, p.stipend, p.skype, p.created_at])
...
def scholarship_csv(request):
    ...
    if user_.is_irc:
        ...
        writer.writerow(['name', 'Scholarship_name', 'Roll_Number', 'Department', 'Email', 'FacAd', 'HOD', 'Host_Institution', 'Year_of_Graduation', 'DOV(from)', 'DOV(to)', 'proposed research/professional internship', 'SOP', 'History of Collaboration (if any) with the host institution/company', 'Justification of applying through (IITKGP) USA Award Program', 'Possible outcome of your proposed research/ professional internship', 'How your engagement with the Host Institute/company will be beneficial to IIT KGP', 'CV', 'Awards', 'Transcript', 'NOC', 'LOE', 'Work Publication', 'fuding proof', 'offer Letter', 'Reviewed by'])
        ...
        for p in application:
            ...
            if p.letter_of_endorsement:
                loe = 'http://ircell.iitkgp.ac.in' + str(p.letter_of_endorsement.url)
            else:
                loe = None
            ...
            if p.fuding_proof:
                ...
                fp = None
            writer.writerow([p.name, p.scholarship.scholarship_name, p.roll, p.department, p.email, p.NameofFacultyAdvisor, p.NameofHOD, p.Nameofhostinstitution, p.yearOfGrad, p.DurationOfVisit_from, p.DurationOfVisit_to, p.DetailsOfTheProposedResearch_or_professionalInternship, p.sop, p.HistoryOfCollobration, p.Justification_of_applying_through_the_IITKGF_Of_USA_AwardProgram, p.DetailsOfTheProposedResearch_or_professionalInternship, p.Explain_in_about_250_words_how_your_engagement_with_the_Host_Institute, 'https://ircell.iitkgp.ac.in' + p.cv.url, awards, 'https://ircell.iitkgp.ac.in' + p.Transcript.url, 'https://ircell.iitkgp.ac.in' + p.NOC.url, loe, wp, fp, 'https://ircell.iitkgp.ac.in' + p.offer_letter.url, p.reviewed_by])
