...
def home(request):
    ...
    return render(request, 'index.html', {'events': events, 'testimonials_irc': testimonials_irc, 'team22': team22})
...
def index(request):
    ...
    if not notices:
        success = False
    return render(request, 'index.html', {'success': success, 'pendingNotifications': pendingNotifications, 'notices': notices, 'testimonials_ftp': testimonials_ftp, 'team3': team3})
...
def view_projects_new(request):
    ...
    return render(request, 'projects.html', {"success": success, 'allprojects': rem_project, 'prof': profile, 'bookmarks': bookmarked, 'pastproject': pastproject, 'proj_domains': proj_domains, 'date': current_time, 'selected': profile.selected, "pendingNotifications": pendingNotifications})
...
def dashboard_mail(request):
    ...
    return render(request, 'projects.html', {'displaylist': display, 'displaylist_past': display_past, 'allprofessors': professors, 'prof': profile, 'projects_declared': projects_declared})
...
def admin_projects(request):
    ...
    if user.is_irc:
        ...
        for result in results:
            projects_declared.append(result.project)
        print(projects_declared)
        return render(request, 'projects.html', {'displaylist': display, 'displaylist_past': display_past, 'allprofessors': professors, 'prof': profile, 'projects_declared': projects_declared, 'projtag': project_tag})
    else:
        raise Http404('Page does not Exist')
